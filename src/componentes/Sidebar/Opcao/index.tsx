...
export default function Opcao({label, icon, onPress, selecionado}: OpcaoProps) {
 ...
 return (
   <TouchableOpacity onPress={onPress} activeOpacity={0.6}>
        { selecionado ? (
            <View className='h-10 mx-3 m-0.5 rounded-2xl bg-yellow-500 flex-row gap-2 items-center p-4'>
                <Ionicons name={icon} size={20} color="white" />
                <Text className='text-sm font-semibold text-white'>{label}</Text>
            </View>
        ) : (
            <View className='h-10 mx-3 m-0.5 flex-row gap-2 items-center p-4'>
